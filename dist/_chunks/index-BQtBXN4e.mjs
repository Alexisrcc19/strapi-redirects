...
const index = {
  register(app) {
    app.addMenuLink({
      ...
      Component: async () => {
        const { App } = await import("./App-Dn5fPqa4.mjs");
        return App;
      }
    });
      ...
      [
        {
          ...
          Component: async () => {
            const { Settings } = await import("./Settings-BaPNHhY2.mjs");
            return Settings;
          }
        }
      ]
  ...
  async registerTrads(app) {
    ...
    const importedTranslations = await Promise.all(
      locales.map((locale) => {
        return __variableDynamicImportRuntimeHelper(/* @__PURE__ */ Object.assign({ "./translations/en.json": () => import("./en-D1a4YMLo.mjs") }), `./translations/${locale}.json`, 3).then(({ default: data }) => {
          return {
